...
function UpdateUser() {
    ...
    useEffect(() => {
        setLoading(true);
        axios.get(`https://anonymous-feedback-app-k60e.onrender.com/getUser/${id}`)
            .then(result => {
    ...
    const Update = async (e) => {
        ...
        setLoading(true);
        try {
            await axios.put(`https://anonymous-feedback-app-k60e.onrender.com/updateUser/${id}`, { no, name });
            setLoading(false);
            navigate('/edit');
        } catch (error) {
            console.log(error);
